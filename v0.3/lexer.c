...
static TokenType check_keyword(const char* str, int len) {
    ...
    if (len == 4 && strncmp(str, "true", 4) == 0) return TOKEN_BENAR;
    if (len == 5 && strncmp(str, "salah", 5) == 0) return TOKEN_SALAH;
    if (len == 5 && strncmp(str, "false", 5) == 0) return TOKEN_SALAH;
    if (len == 5 && strncmp(str, "kosong", 5) == 0) return TOKEN_NULL;
    if (len == 4 && strncmp(str, "null", 4) == 0) return TOKEN_NULL;
    
    // NEW: For loop keywords
    if (len == 5 && strncmp(str, "untuk", 5) == 0) return TOKEN_UNTUK;
    if (len == 3 && strncmp(str, "for", 3) == 0) return TOKEN_UNTUK;
    if (len == 5 && strncmp(str, "dalam", 5) == 0) return TOKEN_DALAM;
    if (len == 2 && strncmp(str, "in", 2) == 0) return TOKEN_DALAM;
    if (len == 5 && strncmp(str, "range", 5) == 0) return TOKEN_RANGE;
